...
class TestStreamStructure:
    ...
    def test_activity_exists(self):
        """Test that activity method exists with correct signature."""
        # Check method exists
        method = getattr(StreamClient, "activity", None)
        assert method is not None, f"Method activity does not exist on StreamClient"
        # Check method is callable
        assert callable(method), f"activity is not callable"
        # Check method signature
        sig = inspect.signature(method)
        params = list(sig.parameters.keys())
        # Should have 'self' as first parameter
        assert len(params) >= 1, f"activity should have at least 'self' parameter"
        assert (
            params[0] == "self"
        ), f"First parameter should be 'self', got '{params[0]}'"
        # Check required parameters exist (excluding 'self')
        ...
        for required_param in required_params:
            assert (
                required_param in params
            ), f"Required parameter '{required_param}' missing from activity"
        # Check optional parameters have defaults (excluding 'self')
        optional_params = [
            "backfill_minutes",
            "start_time",
    ...
    def test_activity_return_annotation(self):
        """Test that activity has proper return type annotation."""
        method = getattr(StreamClient, "activity")
        sig = inspect.signature(method)
        # Check return annotation exists
        assert (
            sig.return_annotation is not inspect.Signature.empty
        ), f"Method activity should have return type annotation"
