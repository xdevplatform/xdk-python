...
class TestBookmarksContracts:
    ...
    def test_create_users_bookmark_folder_request_structure(self):
        """Test create_users_bookmark_folder request structure."""
        # Mock the session to capture request details
        with patch.object(self.client, "session") as mock_session:
            mock_response = Mock()
            ...
            # Call the method
            try:
                method = getattr(self.bookmarks_client, "create_users_bookmark_folder")
                # Check if this is a true streaming operation (has stream_config parameter)
                import types
                import inspect
                sig = inspect.signature(method)
                ...
                if (
                    ...
                    pytest.fail(
                        f"Contract test failed for create_users_bookmark_folder: {e}"
                    )


    def test_create_users_bookmark_folder_required_parameters(self):
        """Test that create_users_bookmark_folder handles parameters correctly."""
        method = getattr(self.bookmarks_client, "create_users_bookmark_folder")
        # Test with missing required parameters - mock the request to avoid network calls
        with patch.object(self.client, "session") as mock_session:
            # Mock a 400 response (typical for missing required parameters)
    ...
    def test_create_users_bookmark_folder_response_structure(self):
        """Test create_users_bookmark_folder response structure validation."""
        with patch.object(self.client, "session") as mock_session:
            # Create mock response from the OpenAPI response schema (minimal valid payload)
            mock_response_data = json.loads(r"""{}""")
            mock_response = Mock()
