...
class TestActivityContracts:
    ...
    def test_delete_subscriptions_by_ids_request_structure(self):
        """Test delete_subscriptions_by_ids request structure."""
        # Mock the session to capture request details
        with patch.object(self.client, "session") as mock_session:
            mock_response = Mock()
            mock_response.status_code = 200
            ...
            # Call the method
            try:
                method = getattr(self.activity_client, "delete_subscriptions_by_ids")
                # Check if this is a true streaming operation (has stream_config parameter)
                import types
                import inspect
                sig = inspect.signature(method)
                ...
                if (
                    ...
                    pytest.fail(
                        f"Contract test failed for delete_subscriptions_by_ids: {e}"
                    )


    def test_delete_subscriptions_by_ids_required_parameters(self):
        """Test that delete_subscriptions_by_ids handles parameters correctly."""
        method = getattr(self.activity_client, "delete_subscriptions_by_ids")
        # Test with missing required parameters - mock the request to avoid network calls
        with patch.object(self.client, "session") as mock_session:
            # Mock a 400 response (typical for missing required parameters)
    ...
    def test_delete_subscriptions_by_ids_response_structure(self):
        """Test delete_subscriptions_by_ids response structure validation."""
        with patch.object(self.client, "session") as mock_session:
            # Create mock response from the OpenAPI response schema (minimal valid payload)
            mock_response_data = json.loads(r"""{}""")
            mock_response = Mock()
    ...
    def test_update_subscription_request_structure(self):
        ...
        with patch.object(self.client, "session") as mock_session:
            ...
            kwargs = {}
            # Add required parameters
            kwargs["subscription_id"] = "test_subscription_id"
            # Add request body if required
            # Import and create proper request model instance
