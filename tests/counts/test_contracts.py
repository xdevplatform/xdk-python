...
"""
Auto-generated contract tests for {"class_name": "Counts", "display_name": "counts", "import_name": "counts", "original": ["counts"], "property_name": "counts"} client.

This module contains tests that validate the request/response contracts
of the {"class_name": "Counts", "display_name": "counts", "import_name": "counts", "original": ["counts"], "property_name": "counts"} client against the OpenAPI specification.
...
class TestCountsContracts:
    """Test the API contracts of CountsClient."""
    ...
    def test_get_posts_recent_request_structure(self):
        """Test get_posts_recent request structure."""
        ...
        with patch.object(self.client, "session") as mock_session:
            ...
            # Call the method
            try:
                method = getattr(self.counts_client, "get_posts_recent")
                ...
                if (
                    ...
                    pytest.fail(f"Contract test failed for get_posts_recent: {e}")


    def test_get_posts_recent_required_parameters(self):
        """Test that get_posts_recent handles parameters correctly."""
        method = getattr(self.counts_client, "get_posts_recent")
    ...
    def test_get_posts_recent_response_structure(self):
        """Test get_posts_recent response structure validation."""
        with patch.object(self.client, "session") as mock_session:
    ...
    def test_get_posts_all_request_structure(self):
        """Test get_posts_all request structure."""
        # Mock the session to capture request details
        with patch.object(self.client, "session") as mock_session:
            mock_response = Mock()
            mock_response.status_code = 200
            ...
            # Call the method
            try:
                method = getattr(self.counts_client, "get_posts_all")
                # Check if this is a true streaming operation (has stream_config parameter)
                import types
                import inspect
                sig = inspect.signature(method)
                ...
                if (
                    ...
                    pytest.fail(f"Contract test failed for get_posts_all: {e}")


    def test_get_posts_all_required_parameters(self):
        """Test that get_posts_all handles parameters correctly."""
        method = getattr(self.counts_client, "get_posts_all")
        # Test with missing required parameters - mock the request to avoid network calls
        with patch.object(self.client, "session") as mock_session:
            # Mock a 400 response (typical for missing required parameters)
    ...
    def test_get_posts_all_response_structure(self):
        """Test get_posts_all response structure validation."""
        with patch.object(self.client, "session") as mock_session:
            # Create mock response from the OpenAPI response schema (minimal valid payload)
            mock_response_data = json.loads(r"""{}""")
            mock_response = Mock()
